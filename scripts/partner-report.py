...
def fetch_sessions(created_after: datetime | None, partner_filter: str | None) -> list[dict]:
    ...
    for session in stripe.checkout.Session.list(**params).auto_paging_iter():
        ...
        sessions.append({
            ...
            'gross_eur': gross / 100,
            'payout_eur': round(gross * COMMISSION_RATE / 100, 2),
...
def print_summary(sessions: list[dict]) -> None:
    ...
    for s in sessions:
        ...
        if pid not in partners:
            partners[pid] = {'count': 0, 'gross': 0.0, 'payout': 0.0}
    ...
    print('=' * 65)
    print(f'{"Partner":<25} {"Vendite":>8} {"Lordo":>12} {"Da pagare":>12}')
    print('-' * 65)
    total_gross = 0.0
    total_payout = 0.0
    ...
    print('=' * 65)
    print(f'{"TOTALE":<25} {len(sessions):>8} {total_gross:>11.2f}€ {total_payout:>11.2f}€')
    print('=' * 65)
...
def save_csv(sessions: list[dict], path: str) -> None:
    ...
    fields = ['session_id', 'created', 'partner_id', 'product', 'gross_eur', 'payout_eur', 'currency', 'customer_email']
