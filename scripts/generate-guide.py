...
def parse_chapters(raw: str) -> list[Chapter]:
    ...
    for i, block in enumerate(blocks):
        ...
        # First non-empty line is the title; skip header underlines like "----"
...
def resolve_voice_id(voice_arg: str) -> tuple[str, str]:
    """Return (env_var_name, voice_id) or raise."""
    env_var = f"{voice_arg.upper()}_VOICE_ID"
    voice_id = os.getenv(env_var)
    if not voice_id:
        raise SystemExit(
            f"\n[ERR] Missing {env_var} in .env\n"
            f"      Add a line like: {env_var}=xxxxxxxxxxxxxxxxxxxx\n"
            f"      Find the voice id at https://elevenlabs.io/app/voices\n"
        )
...
def main() -> int:
    ...
    print(f"\n=== Localis · {args.slug} · {args.lang} · voice={args.voice} ===")
    ...
    for ch in chapters:
        ...
        print(f"  [{ch.index:02d}] {ch.title:<40} {ch.char_count:>5} chars{suffix}")
    ...
    env_var, voice_id = resolve_voice_id(args.voice)
    api_key = os.getenv("ELEVENLABS_API_KEY")
    if not api_key:
        raise SystemExit("[ERR] Missing ELEVENLABS_API_KEY in .env")

    print(f"\nVoice: {env_var} ({voice_id[:6]}...{voice_id[-4:]})")
    ...
    for ch, sub_chunks in chunk_plan:
        ...
        for sub_idx, sub_text in enumerate(sub_chunks):
            ...
            if chunk_path.exists() and chunk_path.stat().st_size > 1024:
                ...
                print(f"  [gen ] {chunk_name} · {ch.title} · {len(sub_text)} chars ...", end=" ", flush=True)
                t_chunk = time.time()
                generate_chunk_mp3(client, sub_text, voice_id, args.model, chunk_path)
